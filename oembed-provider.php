...
class OembedProviderPlugin {
	...
	public static function add_oembed_links() {
		if ( is_singular() ) {
			echo '<link rel="alternate" type="application/json+oembed" href="' . site_url( '/?oembed=true&amp;format=json&amp;url=' . urlencode( get_permalink() ) ) . '" />' . PHP_EOL;
			echo '<link rel="alternate" type="text/xml+oembed" href="' . site_url( '/?oembed=true&amp;format=xml&amp;url=' . urlencode( get_permalink() ) ) . '" />' . PHP_EOL;
		}
	}
	...
	public static function parse_query( $wp ) {
		...
		if ( array_key_exists( 'format', $wp->query_vars ) &&
			in_array( strtolower( $wp->query_vars['format'] ), $oembed_provider_formats ) ) {
	...
	*/
	...
	public static function render_json( $oembed_provider_data, $wp_query ) {
		...
		if ( version_compare( phpversion(), '5.3.0', '<' ) ) {
			...
			$json_str = json_encode( $oembed_provider_data );
		} else {
			$options = 0;
			// JSON_PRETTY_PRINT added in PHP 5.4
			if ( array_key_exists( 'pretty', $wp_query ) && version_compare( phpversion(), '5.4.0', '>=' ) ) {
			 ...
			 */
			$options = apply_filters( 'oembed_provider_json_options', $options );
			$json_str = json_encode( $oembed_provider_data, $options );
		}

		// add callback if available
		if ( array_key_exists( 'callback', $wp_query ) ) {
			echo $wp_query['callback'] . "($json_str);";
		} else {
			echo $json_str;
		}
